...
@Component({
    selector: 'jhi-language-lbs',
    templateUrl: './language-lbs.component.html'
})
export class LanguageLbsComponent implements OnInit, OnDestroy {

currentAccount: any;
    languages: LanguageLbs[];
    error: any;
    success: any;
    ...
    transition() {
        this.router.navigate(['/language-lbs'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/language-lbs', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: LanguageLbs) {
        return item.id;
    }
