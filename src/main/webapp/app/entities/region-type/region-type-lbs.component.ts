...
@Component({
    selector: 'jhi-region-type-lbs',
    templateUrl: './region-type-lbs.component.html'
})
export class RegionTypeLbsComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/region-type-lbs'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/region-type-lbs', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: RegionTypeLbs) {
        return item.id;
    }
